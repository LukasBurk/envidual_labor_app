...
//    // HAPI HL7-Parser initialisieren
//    val hapiContext: HapiContext = DefaultHapiContext()
//    val pipeParser: PipeParser? = hapiContext.pipeParser

//    //for demo purposes, we just declare a literal message string
//    //for demo purposes, we just declare a literal message string
//    val ackMessageString = "MSH|^~\\&|foo|foo||foo|200108151718||ACK^A01^ACK|1|D|2.4|\rMSA|AA\r"
//
//    //instantiate a PipeParser, which handles the "traditional encoding"
//
//    //instantiate a PipeParser, which handles the "traditional encoding"
//    val pipeParser = PipeParser()
//
//    try {
//        //parse the message string into a Message object
//        val message = pipeParser.parse(ackMessageString)
//
//        //if it is an ACK message (as we know it is),  cast it to an
//        // ACK object so that it is easier to work with, and change a value
//        if (message is ACK) {
//            message.msh.processingID.processingMode.value = "P"
//        }
//
//        //instantiate an XML parser
//        val xmlParser: XMLParser = DefaultXMLParser()
//
...
//        println("''''''''''''''''''''''''''''''''''''''")
//        println(ackMessageInXML)
//    } catch (e: Exception) {
//        e.printStackTrace()
//    }

    try {
        println("Hallooo Lukas")
        // Pfad zur vorhandenen HL7-Datei
        val hl7FilePath = "0555890287.hl7"
        //val fullPath = Path("C:\\Users\\lukas\\AndroidStudioProjects\\envidual_labor_app\\app\\src\\main\\java\\com\\example\\envidual_labor_app\\ui\\theme\\data\\0555890287.hl7").absolutePathString()
        // HL7-Datei einlesen
        //val fileread = FileReader(fullPath)


        //val fileContent = getResourceAsText("/res/data/0555890287.hl7")

        //println("PATHHHHHHHHHHH: $fileContent")
//        val bufferedReader: BufferedReader = File("0555890287.txt").bufferedReader()
//        val inputString = bufferedReader.use { it.readText() }
//        println(inputString)


        //val hl7File = File(hl7FilePath)
        //val stringHL = hl7File.readText()
        //val stringHL = hl7File.toString()
        //println(stringHL)
        //val hl7MessageString = hl7File.readText()

//        // HAPI-Parser initialisieren
//        val context: HapiContext = DefaultHapiContext()
//        val parser: Parser = context.getPipeParser()
//
        val parser = PipeParser()
        val messagen = parser.parse(textform)

        val patientename = messagen["PID"]

        println("HALOOOO: $patientename")


//
//        val messageType = messagen.parent
//
//
//        // Sie können nun auf die Daten in der Nachricht zugreifen
////        val patientName = messagen.get("/PID-3-1")
////        val patientID = messagen.get("/PID-3-1")
//
//        //val patientName = messagen.getAll("PID")
//        //val patientID = messagen.get

        // HL7-Nachricht parsen
//        val messagen: Message = pipeParser!!.parse(message)
//
//        // Das gewünschte Segment (z.B., PID) auswählen
//        val pidSegmentt = message.
//
//        // Den gewünschten Wert aus dem Segment extrahieren
//        val patientName = pidSegment[5].toString()


        //println("Patientenname: $patientName")
        //println("Patienten-ID: $patientID")
    } catch (e: HL7Exception) {
        e.printStackTrace()
    }
}
//
//fun extractPatientName(hl7Message: String): String? {
//    val segments = hl7Message.split("\r")
//    for (segment in segments) {
//        val fields = segment.split("|")
//        if (fields.isNotEmpty() && fields[0] == "PID") {
//            val field5 = fields.getOrNull(4)
//            if (field5 != null) {
//                val nameComponents = field5.split("^").filter { it.isNotBlank() }
//                if (nameComponents.isNotEmpty()) {
//                    val cleanedName = nameComponents.joinToString(" ")
